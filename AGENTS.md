...
macOS is installed in its own job rather than that matrix, since a Mac cannot be a container: `build-macos` mounts the `.dmg` it just produced, copies the app into `/Applications`, ejects, and runs the installed binary. It asserts the embedded frontend there too. Two things it does not cover: Gatekeeper, because a file CI produced carries no quarantine attribute, so the `xattr -d` step the README gives users is never exercised; and the Intel slice, which `lipo` and `otool` verify but no runner executes.
