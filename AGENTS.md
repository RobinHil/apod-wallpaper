...
`release` waits on `build-macos`, `install-macos`, `build-linux`, `build-arch` and `install-linux`. A tag is publishable only once every package has been built and installed somewhere.
