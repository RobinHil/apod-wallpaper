...
The version is declared four times: `package.json`, `src-tauri/Cargo.toml`, `src-tauri/tauri.conf.json` and `packaging/aur/PKGBUILD`. CI fails if they disagree, and if a `v*` tag does not match them. Change all four together.
...
Arch has no Tauri bundler, so `packaging/aur/PKGBUILD` assembles the package itself, from `pnpm tauri build --no-bundle` plus the desktop entry beside it. Through the CLI rather than `cargo build --release`, and that is not a stylistic preference: Tauri decides between a development and a production build from the `custom-protocol` feature, which the CLI sets and a bare cargo invocation does not, so `cargo build --release` yields a binary that loads its panel from `devUrl` and, once installed, reports that it cannot reach localhost. Nothing warns about it, which is why `build-arch` exists in CI: it builds the PKGBUILD in an `archlinux:base-devel` container, from the checkout rather than from the published tag, then unpacks the package and fails if the binary embeds no `index-*.js`. That assertion is the whole point of the job; a development build is valid in every other respect. That desktop entry is deliberately not `NoDisplay`: without a tray, launching the app from the overview is the only route to the panel that always works. The PKGBUILD builds the published tag, not the working tree, so it only succeeds against a tag that already carries `pnpm-lock.yaml`.

`bundle.linux.deb.depends` and `bundle.linux.rpm.depends` are package names written by hand, and the RPM ones are Fedora's although the package is built on Ubuntu. `install-linux` in CI is what resolves them: each case starts from a stock image of the distribution the package targets and lets its own package manager install it, then runs `--version`, which loads every shared library the binary names without needing a display. Four cases: the `.deb` under `debian:stable-slim`, the `.rpm` under `fedora:latest`, the AppImage on Fedora, and the Arch package under `archlinux:base`. That last one means more than the same step inside build-arch would: there `makepkg -s` had installed every dependency before the package existed, so nothing proved they resolve from a clean system, and `pacman -U` on a stock image does. The AppImage case installs seventeen sonames first, because an AppImage deliberately omits what the AppImage excludelist calls host-provided -- libfribidi, GLib, Pango, libGL -- so a bare container fails on a missing `libfribidi.so.0` and proves nothing. That list is not a guess: walking the published AppImage's NEEDED entries transitively gives 100 libraries it carries and 21 it wants from the host, less the four glibc always has. It doubles as an assertion, since a bundle that starts wanting something else fails the job rather than being handed it. What stays uninstalled is webkit2gtk, libsoup and GStreamer, which is what the bundle must supply. It asks only whether it starts away from a Debian-shaped host; `--version` exits before GStreamer is reached, so a codec missing from the bundle is not something this catches. Nothing is compiled in that job, and it is the only place a glibc other than the runner's ever sees this binary.

macOS is installed in its own job rather than that matrix, since a Mac cannot be a container. Producing and installing are split in two: `build-macos` compiles, `lipo`s and inspects the bundle, and `install-macos` takes the `.dmg` back out of the artifact store -- byte for byte what a release attaches -- mounts it, copies the app into `/Applications`, ejects, and runs the installed binary. It asserts the embedded frontend there too. Two things it does not cover: Gatekeeper, because a file CI produced carries no quarantine attribute, so the `xattr -d` step the README gives users is never exercised; and the Intel slice, which `lipo` and `otool` verify but no runner executes.

`release` waits on `install-macos` and `install-linux`, which is the whole graph: each install job already waits on what built what it installs, so the three build jobs gate a publication transitively. A tag is publishable only once every package has been built and installed somewhere.
